...
def main() -> None:
    ...
    Image.fromarray(overlay).save(config.OUTPUT / "sam2_auto_overlay.jpg", quality=92)
    ...
    for ax, ann in zip(axes.ravel(), top):
        ...
        ax.axis("off")
    fig.tight_layout()
    fig.savefig(config.OUTPUT / "sam2_auto_top_masks.png", dpi=160)
    plt.close(fig)
    ...
    (config.OUTPUT / "sam2_auto_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
