...
def compare_sizes(results: list[dict]) -> None:
    ...
    for ax, (size, im) in zip(axes, imgs):
        ...
        ax.axis("off")
    fig.tight_layout()
    fig.savefig(config.OUTPUT / "cnn_patch_size_compare.png", dpi=160)
    plt.close(fig)
...
def main() -> None:
    ...
    draw_legend(config.OUTPUT / "cnn_legend.png")
    ...
    if args.compare_all:
        ...
        (config.OUTPUT / "cnn_infer_report.json").write_text(
    ...
    print(f"Saved {config.OUTPUT / out}")
