...
def main() -> None:
    ...
    fig.tight_layout()
    fig.savefig(config.SAM_OUTPUT / "sam2_parameter_sweep.png", dpi=160)
    plt.close(fig)

    (config.SAM_OUTPUT / "sam2_sweep_report.json").write_text(
        json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"Saved {config.SAM_OUTPUT / 'sam2_parameter_sweep.png'}")
