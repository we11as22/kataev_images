...
def draw_annotations_preview(image: np.ndarray, scale: float) -> None:
    ...
    preview.save(config.OUTPUT / "annotations_preview.jpg", quality=92)
...
def draw_legend() -> None:
    ...
    fig.tight_layout()
    fig.savefig(config.OUTPUT / "legend.png", dpi=160, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16, enabled=torch.cuda.is_available()):
        ...
        Image.fromarray(image).save(config.OUTPUT / "source_resized.jpg", quality=92)
        Image.fromarray(overlay).save(config.OUTPUT / "sam2_prompt_overlay.jpg", quality=92)
        ...
        for idx, cls in enumerate(config.CLASSES):
            ...
            axes[idx].axis("off")
        fig.tight_layout()
        fig.savefig(config.OUTPUT / "sam2_prompt_classes.png", dpi=160)
        plt.close(fig)
    ...
    (config.OUTPUT / "sam2_prompt_report.json").write_text(
